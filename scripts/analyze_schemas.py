...
PLAYER_FACING_FIELD = re.compile(
    ...
    r"RequirementFriendlyDescription)",
    re.IGNORECASE,
)
...
def parse_dump_classes(path: Path) -> list[dict]:
    ...
    ctor_re = re.compile(r"^\s*(?:public|private|internal|protected)\s+([A-Za-z0-9_]+)\s*\(")
...
def main():
    ...
    # Terminal report
    print(f"\n{'=' * 70}")
    print("GorgonCore CLASS ANALYSIS")
    print(f"{'=' * 70}")
    ...
    print(f"{'─' * 70}")
    print("CLASSES WITH PLAYER-FACING TEXT FIELDS")
    print(f"{'─' * 70}")
    for c in candidates:
        ...
        print(
            f"    Static: {c['is_static']}, Has ctor: {c['has_ctor']}, JSON ctor: {c['has_json_ctor']}"
        )
        ...
        for f in c["player_facing_fields"]:
            ...
            in_cdn = (
                ...
                else ("✗ MISSING" if field_name in missing else "?")
            )
            print(f"      - {f.split('//')[0].strip():55s} {in_cdn}")

    print(f"\n{'─' * 70}")
    print(
        f"CLASSES WITHOUT PLAYER-FACING TEXT FIELDS ({len([c for c in candidates if not c['player_facing_fields']])})"
    )
    print(f"{'─' * 70}")
    for c in candidates:
    ...
    with open(DOC_PATH, "w", encoding="utf-8") as f:
        ...
        f.write("Generated by `scripts/analyze_schemas.py`\n")
        ...
        for c in candidates:
            ...
            f.write(
                f"| {c['name']} | {c['field_count']} | {field_names} | {cdn_str} | {cdn_covers} |\n"
            )
        ...
        for c, missing, _cdns in recommendations:
            cdn_status = (
                f"CDN missing: {', '.join(missing)}" if missing else "No CDN reference found"
            )
            f.write(
                f"| {c['name']} | {len(c['player_facing_fields'])} player-facing fields | {cdn_status} | \n"
            )
    ...
    print(
        f"Candidate classes with player-facing fields: {sum(1 for c in candidates if c['player_facing_fields'])}"
    )
