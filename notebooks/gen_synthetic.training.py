...
def gen_synthetic(
    ...
    Run after cell order: imports -> teacher -> load_student -> gen_synthetic.
    All imports provided by the shared imports cell.
    Hardware: molab-class GPU assumed (CUDA cuda:0, 4-bit 27B teacher; BATCH=8 needs ~102 GB class VRAM for left-padded batch generate).
    """
    ...
    # Lever 4: batch docs per generate() call. VRAM headroom measured mid-run:
    # 39.2/102 GB used, so 8-way left-padded batching is safe.
    BATCH = 8
    ...
    if not to_process:
        ...
        for i, doc in enumerate(to_process):
            ...
            try:
                texts = []
                for _sb in sub_batch:
                    messages = [{"role": "user", "content": _sb[5]}]
                    text = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True, enable_thinking=False)
                    # Pre-fill "Question: " in the assistant turn so the model continues from there
                    text = text + "Question: "  # model generates: <question text>\nAnswer: <answer text>
                    texts.append(text)
                ...
                if retry_pending:
                    ...
                    for _sb in retry_pending:
                        ...
                        texts2.append(text2)
                    enc2 = tokenizer(texts2, return_tensors="pt", padding=True, truncation=True, max_length=4096)
                    inputs2 = {k: v.cuda() for k, v in enc2.items()}
                    with torch.no_grad():
                        outputs2 = teacher_model.generate(
                            **inputs2,
                            max_new_tokens=GEN_MAX_TOKENS,
                            do_sample=False,
                            pad_token_id=tokenizer.pad_token_id,
