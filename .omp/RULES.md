...
14. **Cross-instance build lock.** `.omp/hooks/pre/00-repo-lock.ts` guards destructive corpus commands (`build-*`, `generate-docs`, `mise refresh/sync`) with an `O_EXCL` lock at `data/.omp-build.lock`. Never bypass it by hand-editing the lock; to clear a stuck lock, delete `data/.omp-build.lock`.