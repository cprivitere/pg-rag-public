...
(`https://sb-XXXX.sb.molab.run/`). Thin layer over the `marimo-pair-win`
skill's `execute-code.ps1`; adds a persistent per-sandbox credential cache so
the user never has to re-paste the URL + token.
...
## Reference
- All code execution is delegated to `marimo-pair-win`'s
  `execute-code.ps1` (same skill directory family); read its SKILL.md /
  gotchas for the cm API, stdin piping, and cell-editing details.
